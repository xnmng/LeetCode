...
public:
    Bank(vector<long long>& balance) : v(balance) {
        // no need to check account > 0 because of constraints!
    }
    
    bool transfer(int account1, int account2, long long money) {
        // need to first check validity of both accounts!!
        if (account2 <= v.size() && withdraw(account1, money)) {
    ...
    bool withdraw(int account, long long money) {
        if (account > v.size() || v[account-1] < money) return false;
        v[account-1] -= money;
        return true;
    }
};
