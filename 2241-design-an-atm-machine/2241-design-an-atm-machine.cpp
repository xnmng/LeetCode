...
private:
    ...
    vector<long long> amt{20, 50, 100, 200, 500};
public:
    ATM() : notes(5, 0) {}
    ...
    vector<int> withdraw(int amount) {
        ...
        while (amount > 0 && index >= 0) {
            if (notes[index] > 0 && amount >= amt[index]) {
                ans[index] = min(notes[index], amount / amt[index]);
                amount -= ans[index] * amt[index];
            }
            --index;
        }
        if (amount > 0) return {-1};
        for (auto i = 0; i < 5; ++i) notes[i] -= ans[i];
        return ans;
    }
};
